...
export const useSprintStore = create<SprintState>((set, get) => ({
  ...
  syncData: async () => {
    set({ isLoading: true });
    try {
      const [sprints, tracking, reviews] = await Promise.all([
        api.fetchSprints(),
        api.fetchTracking(),
        api.fetchReviews()
      ]);

      const sprintData = Array.isArray(sprints) ? sprints : [];
      const trackingData = Array.isArray(tracking) ? tracking : [];
      const reviewData = Array.isArray(reviews) ? reviews : [];

      const mappedSprints: Sprint[] = sprintData.map((s: any) => ({
        id: s.id,
        name: s.name,
        goal: s.goal,
      ...
      const mappedReviews: WeeklyReview[] = reviewData.map((r: any) => ({
        id: r.id,
        sprintId: r.sprint_id,
        weekNumber: r.week_number,
        whatWorked: r.what_worked,
        whatFailed: r.what_failed,
        biggestDistraction: r.biggest_distraction,
        nextWeekFocus: r.next_week_focus,
        date: r.date
      }));

      const active = mappedSprints.find(s => s.isActive);
      
      set({ 
        sprints: mappedSprints,
        activeSprint: active || null,
        dailyTracking: trackingData,
        weeklyReviews: mappedReviews
      });
    } catch (error) {
